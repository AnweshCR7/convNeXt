...
def run_training():
    ...
    train_set = datasets.EMNIST(root=config.DATA_DIR, train=True, transform=simple_transform, download=True, split='balanced')
    test_set = datasets.EMNIST(root=config.DATA_DIR, train=False, transform=simple_transform, download=True, split='balanced')
